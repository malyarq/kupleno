...
function isCollectionCompletenessWarning(value) {
  return /состав не распознан|без состава|не разобрано чеков|без сверки итога|не распознан итог|полнота состава не подтверждена|пропущено повреждённых чеков|достигнут лимит страниц|список (?:заказов|чеков) загружен не полностью/i
    .test(String(value || ''));
}
